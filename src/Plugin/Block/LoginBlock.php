...
class LoginBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $build['#attached'] = [
        'library' => [
          'oe_corporate_blocks/login',
        ],
      ];

    $config = $this->configFactory->get('oe_corporate_blocks.data.login');

    NestedArray::setValue($build, ['#corporate_login', 'login', 'title'], $config->get('login_title'));
